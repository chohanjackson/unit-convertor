...
#Build a Google Unit Convertor using Pyhton and Streamline:
...
st.markdown(
   ...
   .stButton>butto:hover {
       transform: scale(1.05);
       background: linear-gradient(45deg, #0b5394, #00c9ff);
       color: black;
   } 
   .result-box {
        font-size: 20px
...
#sidebar menu
conversion_type = st.sidebar.selectbox("Choose Conversion Type", ["Length", "Weight", "Temperture"])

# Input value (user se value lene ke liye)
value =st.number_input("Enter Value", value=0.0, min_value=0.0, step=0.1)

# Layout for unit selection (from aur to units select krne ka setup)
...
elif conversion_type == "Temperture":
    with col1:
        from_unit = st.selectbox("From", ["Celsis", "Fahrenenteit", "Kelvin"])
    with col2:
        to_unit = st.selectbox("To",  ["Celsis", "Fahrenheit", "Kelvin"])

#converted function (Alag alag conversions ke liye functions)
def length_convertor(value, from_unit, to_unit):
    # Length units ko conversion factor ke zariye convert karna
    length_units = {
        'Meters': 1, 'Kilometers': 0.001, 'Centimeters': 100, 'Millimetrs': 1000,
...
if st.button("🤖Convert"):
    ...
    #Result ko dikhane (Conversion ka result show krna)
    st.markdown(f"<div class='result-box'>{value} {from_unit} = {result: 4f} {to_unit}</div>",unsafe_allow_html=True)

# Footer (Neehy footer message)     
st.markdown("<div class='footer'>Created with love by Jackson Chohan /div", unsafe_allow_html=True)
